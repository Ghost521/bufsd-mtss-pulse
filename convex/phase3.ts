...
export const listStudentProfileRows = query({
  args: {
    tenantKey: v.string(),
  },
  handler: async (ctx, args) => {
    const rows = await ctx.db
      .query("tenantStudentProfileRecords")
      .withIndex("by_tenant")
      .filter((q) => q.eq(q.field("tenantKey"), args.tenantKey))
      .collect();
...
export const listLessonPlanRows = query({
  args: {
    tenantKey: v.string(),
  },
  handler: async (ctx, args) => {
    const rows = await ctx.db
      .query("tenantLessonPlanRecords")
      .withIndex("by_tenant")
      .filter((q) => q.eq(q.field("tenantKey"), args.tenantKey))
      .collect();
...
export const listStaffRows = query({
  args: {
    tenantKey: v.string(),
  },
  handler: async (ctx, args) => {
    const rows = await ctx.db
      .query("tenantStaffRecords")
      .withIndex("by_tenant")
      .filter((q) => q.eq(q.field("tenantKey"), args.tenantKey))
      .collect();
...
export const replaceImportRows = mutation({
  args: {
    tenantKey: v.string(),
    rows: v.array(v.any()),
  },
  handler: async (ctx, args) => {
    ...
    for (let index = 0; index < args.rows.length; index += 1) {
      ...
      const candidate = row as Record<string, unknown>;
      if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) continue;
      await ctx.db.insert("tenantImportRecords", {
        tenantKey: args.tenantKey,
...
export const replaceStaffRows = mutation({
  args: {
    tenantKey: v.string(),
    rows: v.array(v.any()),
  },
  handler: async (ctx, args) => {
    ...
    for (let index = 0; index < args.rows.length; index += 1) {
      ...
      const candidate = row as Record<string, unknown>;
      if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) continue;
      await ctx.db.insert("tenantStaffRecords", {
        tenantKey: args.tenantKey,
...
export const replaceLessonPlanRows = mutation({
  args: {
    tenantKey: v.string(),
    rows: v.array(v.any()),
  },
  handler: async (ctx, args) => {
    ...
    for (let index = 0; index < args.rows.length; index += 1) {
      ...
      const candidate = row as Record<string, unknown>;
      if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) continue;
      await ctx.db.insert("tenantLessonPlanRecords", {
        tenantKey: args.tenantKey,
        lessonPlanId: candidate.id,
        row,
        position: index,
        updatedAt: now,
      });
...
export const replaceStudentProfileRows = mutation({
  args: {
    tenantKey: v.string(),
    rows: v.array(v.any()),
  },
  handler: async (ctx, args) => {
    ...
    for (let index = 0; index < args.rows.length; index += 1) {
      ...
      const candidate = row as Record<string, unknown>;
      if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) continue;
      await ctx.db.insert("tenantStudentProfileRecords", {
        tenantKey: args.tenantKey,
...
export const upsertStudentProfileRow = mutation({
  args: {
    tenantKey: v.string(),
    row: v.any(),
    position: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    const candidate = args.row as Record<string, unknown>;
    if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) {
      throw new Error("Student profile row must include a string id.");
    }
    const studentId = candidate.id;

    const existing = await ctx.db
      .query("tenantStudentProfileRecords")
      .withIndex("by_tenant_student")
      .filter((q) =>
        q.and(
          q.eq(q.field("tenantKey"), args.tenantKey),
    ...
    return await ctx.db.insert("tenantStudentProfileRecords", next);
  },
});

export const upsertLessonPlanRow = mutation({
  args: {
    tenantKey: v.string(),
    row: v.any(),
    position: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    const candidate = args.row as Record<string, unknown>;
    if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) {
      throw new Error("Lesson plan row must include a string id.");
    }
    const lessonPlanId = candidate.id;

    const existing = await ctx.db
      .query("tenantLessonPlanRecords")
      .withIndex("by_tenant_lesson_plan")
      .filter((q) =>
        q.and(
          q.eq(q.field("tenantKey"), args.tenantKey),
          q.eq(q.field("lessonPlanId"), lessonPlanId),
        )
      )
      .unique();
    ...
    return await ctx.db.insert("tenantLessonPlanRecords", next);
  },
});

export const upsertStaffRow = mutation({
  args: {
    tenantKey: v.string(),
    row: v.any(),
    position: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    const candidate = args.row as Record<string, unknown>;
    if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) {
      throw new Error("Staff row must include a string id.");
    }
    const staffId = candidate.id;

    const existing = await ctx.db
      .query("tenantStaffRecords")
      .withIndex("by_tenant_staff")
      .filter((q) =>
        q.and(
          q.eq(q.field("tenantKey"), args.tenantKey),
    ...
    return await ctx.db.insert("tenantStaffRecords", next);
  },
});

export const upsertImportRow = mutation({
  args: {
    tenantKey: v.string(),
    row: v.any(),
    position: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    const candidate = args.row as Record<string, unknown>;
    if (typeof candidate.id !== "string" || candidate.id.trim().length === 0) {
      throw new Error("Import row must include a string id.");
    }
    const importId = candidate.id;

    const existing = await ctx.db
      .query("tenantImportRecords")
      .withIndex("by_tenant_import")
      .filter((q) =>
        q.and(
          q.eq(q.field("tenantKey"), args.tenantKey),
          q.eq(q.field("importId"), importId),
        )
      )
      .unique();
    ...
    return await ctx.db.insert("tenantImportRecords", next);
  },
});
...
export const deleteImportRow = mutation({
  args: {
    tenantKey: v.string(),
    importId: v.string(),
  },
...
export const deleteStaffRow = mutation({
  args: {
    tenantKey: v.string(),
    staffId: v.string(),
  },
...
export const deleteLessonPlanRow = mutation({
  args: {
    tenantKey: v.string(),
  ...
  handler: async (ctx, args) => {
    const existing = await ctx.db
      ...
      .filter((q) =>
        q.and(
          q.eq(q.field("tenantKey"), args.tenantKey),
          q.eq(q.field("lessonPlanId"), args.lessonPlanId),
        )
      )
      .unique();
...
export const deleteStudentProfileRow = mutation({
  args: {
    tenantKey: v.string(),
    studentId: v.string(),
  },
