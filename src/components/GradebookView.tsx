...
const WeightDistributionChart = lazy(() =>
  import('./GradebookCharts').then((module) => ({ default: module.WeightDistributionChart })),
);
const GradeTrendSparkline = lazy(() =>
  import('./GradebookCharts').then((module) => ({ default: module.GradeTrendSparkline })),
);
...
const WeightChartFallback: React.FC = () => (
  <div className="relative grid h-40 w-40 shrink-0 place-items-center overflow-hidden">
    <div className="h-32 w-32 animate-pulse rounded-full border-[16px] border-slate-200" />
  </div>
);

const TrendSparklineFallback: React.FC = () => (
  <div className="h-6 w-full px-2 opacity-70">
...
export const GradebookView: React.FC<GradebookViewProps> = ({ onMenuClick }) => {
  ...
  const getScore = useCallback((studentId: string, assignmentId: string): ParsedGradeScore => {
  ...
  const mobileScore = useMemo(
    ...
    [getScore, mobileAssignment, mobileStudent],
  );
  ...
  }, [filteredAssignments, getScore, mobileStudent]);
        ...
        <div onKeyDown={handleWeightsModalKeyDown} className="h-full space-y-5 overflow-y-auto p-5 sm:p-6">
          ...
          <div className="grid grid-cols-1 items-start gap-6 md:grid-cols-[180px_minmax(0,1fr)]">
            <div className="flex w-full justify-center">
              <Suspense fallback={<WeightChartFallback />}>
                <WeightDistributionChart
                  tempWeights={tempWeights}
                  colors={COLORS}
                  totalTempWeight={totalTempWeight}
                            ...
                            <tr key={student.id} className="group hover:bg-indigo-50/30 transition-colors">
                                ...
                                <td className="sticky left-[340px] z-10 border-r border-slate-200 p-0 text-center bg-white group-hover:bg-indigo-50/30 shadow-[4px_0_8px_-4px_rgba(0,0,0,0.1)]">
                                    <div className="flex flex-col items-center justify-center h-full py-1.5">
                                        ...
                                        <Suspense fallback={<TrendSparklineFallback />}>
                                          <GradeTrendSparkline avg={avg} trendData={trendData} />
                                        </Suspense>
                                    </div>
                                </td>
