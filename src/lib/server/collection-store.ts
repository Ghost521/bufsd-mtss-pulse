...
const isImportDomain = (domain: DataDomain): boolean => domain === "imports";
const isLessonPlanDomain = (domain: DataDomain): boolean => domain === "lesson-plans";
...
const isStaffDomain = (domain: DataDomain): boolean => domain === "staff";
const isStudentProfileDomain = (domain: DataDomain): boolean => domain === "student-profiles";
...
const readStudentProfileRows = async <T>(tenantKey: string): Promise<T[]> => {
  const rowLevelRows = parseRowsForDomain<T>("student-profiles", await listStudentProfileRowsByTenant<unknown>(tenantKey), 500);
  if (rowLevelRows.length > 0) return rowLevelRows;

  const legacyRows = await readTenantCollection<unknown>(tenantKey, "student_profiles", () => []);
  const parsedLegacyRows = parseRowsForDomain<T>("student-profiles", ensureArray<T>(legacyRows), 500);
  if (parsedLegacyRows.length > 0) {
    await replaceStudentProfileRowsByTenant(tenantKey, parsedLegacyRows as Array<T & { id: string }>);
  }
  return parsedLegacyRows;
};

const readLessonPlanRows = async <T>(tenantKey: string): Promise<T[]> => {
  const rowLevelRows = parseRowsForDomain<T>("lesson-plans", await listLessonPlanRowsByTenant<unknown>(tenantKey), 500);
  if (rowLevelRows.length > 0) return rowLevelRows;

  const legacyRows = await readTenantCollection<unknown>(tenantKey, "lesson_plans", () => []);
  const parsedLegacyRows = parseRowsForDomain<T>("lesson-plans", ensureArray<T>(legacyRows), 500);
  if (parsedLegacyRows.length > 0) {
    await replaceLessonPlanRowsByTenant(tenantKey, parsedLegacyRows as Array<T & { id: string }>);
  }
  return parsedLegacyRows;
};

const readStaffRows = async <T>(tenantKey: string): Promise<T[]> => {
  const rowLevelRows = parseRowsForDomain<T>("staff", await listStaffRowsByTenant<unknown>(tenantKey), 500);
  if (rowLevelRows.length > 0) return rowLevelRows;

  const legacyRows = await readTenantCollection<unknown>(tenantKey, "staff", () => []);
  const parsedLegacyRows = parseRowsForDomain<T>("staff", ensureArray<T>(legacyRows), 500);
  if (parsedLegacyRows.length > 0) {
    await replaceStaffRowsByTenant(tenantKey, parsedLegacyRows as Array<T & { id: string }>);
  }
  return parsedLegacyRows;
};

const readImportRows = async <T>(tenantKey: string): Promise<T[]> => {
  const rowLevelRows = parseRowsForDomain<T>("imports", await listImportRowsByTenant<unknown>(tenantKey), 500);
  if (rowLevelRows.length > 0) return rowLevelRows;

  const legacyRows = await readTenantCollection<unknown>(tenantKey, "imports", () => []);
  const parsedLegacyRows = parseRowsForDomain<T>("imports", ensureArray<T>(legacyRows), 500);
  if (parsedLegacyRows.length > 0) {
    await replaceImportRowsByTenant(tenantKey, parsedLegacyRows as Array<T & { id: string }>);
  }
  return parsedLegacyRows;
};
...
export const listDomainRows = async <T>(session: SessionContext, domain: DataDomain): Promise<T[]> => {
  ...
  if (isImportDomain(domain)) {
    return readImportRows<T>(tenantKey);
  }
  if (isLessonPlanDomain(domain)) {
    return readLessonPlanRows<T>(tenantKey);
  }
  ...
  if (isStaffDomain(domain)) {
    return readStaffRows<T>(tenantKey);
  }
  if (isStudentProfileDomain(domain)) {
    return readStudentProfileRows<T>(tenantKey);
  }
...
export const replaceDomainRows = async <T>(session: SessionContext, domain: DataDomain, rows: T[]): Promise<T[]> => {
  ...
  if (isImportDomain(domain)) {
    await replaceImportRowsByTenant(tenantKey, next as Array<T & { id: string }>);
    if (next.length === 0) {
      await writeTenantCollection(tenantKey, "imports", []);
    }
    return next;
  }
  if (isLessonPlanDomain(domain)) {
    await replaceLessonPlanRowsByTenant(tenantKey, next as Array<T & { id: string }>);
    if (next.length === 0) {
      await writeTenantCollection(tenantKey, "lesson_plans", []);
    }
    return next;
  }
  if (isDocumentDomain(domain)) {
  ...
  if (isStaffDomain(domain)) {
    await replaceStaffRowsByTenant(tenantKey, next as Array<T & { id: string }>);
    if (next.length === 0) {
      await writeTenantCollection(tenantKey, "staff", []);
    }
    return next;
  }
  if (isStudentProfileDomain(domain)) {
    await replaceStudentProfileRowsByTenant(tenantKey, next as Array<T & { id: string }>);
    if (next.length === 0) {
      await writeTenantCollection(tenantKey, "student_profiles", []);
    }
    return next;
  }
  if (isReferralDomain(domain)) {
...
export const createDomainRow = async <T extends object>(
  ...
  if (isImportDomain(domain)) {
    await upsertImportRowByTenant(toTenantKey(session.activeContext), nextRow as Record<string, unknown> & { id: string }, -Date.now());
    return nextRow as unknown as T;
  }
  if (isLessonPlanDomain(domain)) {
    await upsertLessonPlanRowByTenant(toTenantKey(session.activeContext), nextRow as Record<string, unknown> & { id: string }, -Date.now());
    return nextRow as unknown as T;
  }
  if (isDocumentDomain(domain)) {
  ...
  if (isStaffDomain(domain)) {
    await upsertStaffRowByTenant(toTenantKey(session.activeContext), nextRow as Record<string, unknown> & { id: string }, -Date.now());
    return nextRow as unknown as T;
  }
  if (isStudentProfileDomain(domain)) {
    await upsertStudentProfileRowByTenant(toTenantKey(session.activeContext), nextRow as Record<string, unknown> & { id: string }, -Date.now());
    return nextRow as unknown as T;
  }
  if (isReferralDomain(domain)) {
...
export const updateDomainRow = async <T extends object>(
  ...
  if (isImportDomain(domain)) {
    const rows = await readImportRows<Record<string, unknown>>(tenantKey);
    const current = rows.find((row) => row.id === id);
    if (!current) return null;
    const currentRecord = asRecord(current);
    const patchRecord = asRecord(patch);
    ...
    const nextRow = parseRowForDomain<Record<string, unknown>>(domain, { ...currentRecord, ...patchRecord, id });
    await upsertImportRowByTenant(tenantKey, nextRow as Record<string, unknown> & { id: string });
    return nextRow as unknown as T;
  }
  if (isLessonPlanDomain(domain)) {
    const rows = await readLessonPlanRows<Record<string, unknown>>(tenantKey);
    const current = rows.find((row) => row.id === id);
    if (!current) return null;
    const currentRecord = asRecord(current);
    const patchRecord = asRecord(patch);
    ...
    const nextRow = parseRowForDomain<Record<string, unknown>>(domain, { ...currentRecord, ...patchRecord, id });
    await upsertLessonPlanRowByTenant(tenantKey, nextRow as Record<string, unknown> & { id: string });
    return nextRow as unknown as T;
  }
  if (isDocumentDomain(domain)) {
  ...
  if (isStaffDomain(domain)) {
    const rows = await readStaffRows<Record<string, unknown>>(tenantKey);
    const current = rows.find((row) => row.id === id);
    if (!current) return null;
    const currentRecord = asRecord(current);
    const patchRecord = asRecord(patch);
    ...
    const nextRow = parseRowForDomain<Record<string, unknown>>(domain, { ...currentRecord, ...patchRecord, id });
    await upsertStaffRowByTenant(tenantKey, nextRow as Record<string, unknown> & { id: string });
    return nextRow as unknown as T;
  }
  if (isStudentProfileDomain(domain)) {
    const rows = await readStudentProfileRows<Record<string, unknown>>(tenantKey);
    const current = rows.find((row) => row.id === id);
    if (!current) return null;
    const currentRecord = asRecord(current);
    const patchRecord = asRecord(patch);
    ...
    const nextRow = parseRowForDomain<Record<string, unknown>>(domain, { ...currentRecord, ...patchRecord, id });
    await upsertStudentProfileRowByTenant(tenantKey, nextRow as Record<string, unknown> & { id: string });
    return nextRow as unknown as T;
  }
  if (isReferralDomain(domain)) {
...
export const deleteDomainRow = async <T extends object>(
  ...
  if (isImportDomain(domain)) {
    ...
    if (!target) return null;
    const nextRows = rows.filter((row) => row.id !== id);
    await deleteImportRowByTenant(tenantKey, id);
    if (nextRows.length === 0) {
      await writeTenantCollection(tenantKey, "imports", []);
    }
    return target as unknown as T;
  ...
  if (isLessonPlanDomain(domain)) {
    ...
    if (!target) return null;
    const nextRows = rows.filter((row) => row.id !== id);
    await deleteLessonPlanRowByTenant(tenantKey, id);
    if (nextRows.length === 0) {
      await writeTenantCollection(tenantKey, "lesson_plans", []);
    }
    return target as unknown as T;
  ...
  if (isStaffDomain(domain)) {
    ...
    if (!target) return null;
    const nextRows = rows.filter((row) => row.id !== id);
    await deleteStaffRowByTenant(tenantKey, id);
    if (nextRows.length === 0) {
      await writeTenantCollection(tenantKey, "staff", []);
    }
    return target as unknown as T;
  ...
  if (isStudentProfileDomain(domain)) {
    ...
    if (!target) return null;
    const nextRows = rows.filter((row) => row.id !== id);
    await deleteStudentProfileRowByTenant(tenantKey, id);
    if (nextRows.length === 0) {
      await writeTenantCollection(tenantKey, "student_profiles", []);
    }
    return target as unknown as T;
